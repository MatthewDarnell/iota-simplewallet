...
struct TxLessThan
{
    ...
    bool operator()(const TransactionRecord &a, const QString &b) const
    {
        return a.hash < b.toStdString();
    }
    bool operator()(const QString &a, const TransactionRecord &b) const
    {
        return a.toStdString() < b.hash;
    }
};
...
public:
     ...
     */
    void updateWallet(interfaces::Wallet& wallet, const QString &hash, int status, bool showTransaction)
    {
        qDebug() << "TransactionTablePriv::updateWallet: " + hash + " " + QString::number(status);
        ...
        auto changeType = static_cast<ChangeType>(status);

        if(changeType == ChangeType::CT_UPDATED)
        {
            if(showTransaction && !inModel)
                changeType = ChangeType::CT_NEW; /* Not in model, but want to show, treat as new */
            if(!showTransaction && inModel)
                changeType = ChangeType::CT_DELETED; /* In model, but want to hide, treat as deleted */
        }
        ...
        case ChangeType::CT_NEW:
            ...
            if(showTransaction)
            {
                ...
                interfaces::WalletTx wtx = wallet.getWalletTx(hash.toStdString());
                if(wtx.hash.empty())
                {
...
public:
    ...
    TransactionNotification(std::string _hash, ChangeType _status, bool _showTransaction):
        hash(_hash), status(_status), showTransaction(_showTransaction) {}

    void invoke(QObject *ttm)
    {
        QString strHash = QString::fromStdString(hash);
        qDebug() << "NotifyTransactionChanged: " + strHash + " status= " + QString::number(static_cast<int>(status));
...
static void NotifyTransactionChanged(TransactionTableModel *ttm, const std::string &hash, const ChangeType &changeType)
{
    // Find transaction in wallet
    // Determine whether to show transaction or not (determine this here so that no relocking is needed in GUI thread)
    bool showTransaction = TransactionRecord::showTransaction();

    TransactionNotification notification(hash, changeType, showTransaction);
