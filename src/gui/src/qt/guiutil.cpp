...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter an Iota address"));
...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("iota"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::Ti, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
//    if (info.amount)
//    {
//        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::Ti, info.amount, false, BitcoinUnits::separatorNever));
//        paramCount++;
//    }
...
#endif
    qDebug("Qt %s (%s), plugin=%s (%s)\n", qVersion(), qt_link.c_str(), QGuiApplication::platformName().toStdString().c_str(), plugin_link.c_str());
    qDebug("System: %s, %s\n", QSysInfo::prettyProductName().toStdString().c_str(), QSysInfo::buildAbi().toStdString().c_str());
    for (const QScreen* s : QGuiApplication::screens()) {
        qDebug("Screen: %s %dx%d, pixel ratio=%.1f\n", s->name().toStdString().c_str(), s->size().width(), s->size().height(), s->devicePixelRatio());
    }
}
