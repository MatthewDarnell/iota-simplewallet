...
int create_tables(sqlite3* db) {
  ...
  const char* tables[] = {
    ...
    "CREATE TABLE IF NOT EXISTS address(serial integer primary key autoincrement, address text unique not null, balance text not null default \"0\", offset integer not null, account text not null, is_change integer default 0, spent_from integer not null default 0, used integer default 0, created_at timestamp default current_timestamp, unique(offset, account), foreign key (account) references account(username))",
    "CREATE TABLE IF NOT EXISTS incoming_transaction(serial integer primary key autoincrement, time timestamp not null, amount integer not null, address text references address(address), confirmed integer default 0, bundle text not null, hash text unique not null, created_at timestamp default current_timestamp, unique(bundle, address, amount, time))",
    "CREATE TABLE IF NOT EXISTS outgoing_transaction(serial integer primary key autoincrement, amount integer, dest_address text not null, change_address text references address(address), confirmed integer default 0, trytes text not null, sent integer default 0, bundle text, hash text unique, created_at timestamp default current_timestamp)",
  };
