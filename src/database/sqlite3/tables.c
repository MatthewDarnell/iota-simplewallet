...
int create_tables(sqlite3* db) {
  ...
  const char* tables[] = {
    ...
    "CREATE TABLE IF NOT EXISTS address(serial integer primary key autoincrement, address text unique not null, balance text not null default \"0\", offset integer not null, account text not null, is_change integer default 0, created_at timestamp default current_timestamp, unique(offset, account), foreign key (account) references account(username))",
