...
int delete_account_incoming_transaction(sqlite3* db, const char* username) {
  ...
  char* query = "DELETE FROM incoming_transaction "
                " WHERE address IN "
                " ( "
                " SELECT it.address FROM incoming_transaction it"
  ...
  log_wallet_info("%s: User.(%s) Incoming Transactions Deleted successfully.", __func__, username);
  sqlite3_finalize(stmt);
  return 0;
}
